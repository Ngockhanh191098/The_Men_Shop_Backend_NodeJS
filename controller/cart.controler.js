...
const getCart =  async (req, res) => {
    try {
      const {id}= req.params;
      const {fullName,username,email} = await UserModel.findOne({where:{
        id,
      }});
      const productid = await CartModel.findAll({where:{
        userId : id,
      }});
...
module.exports = {getCart, addProductToCart, deleteCart};